...
public class FtpUtil {
    public static boolean downloadFile(FTPClient ftpClient, String remoteFileName, String localDires,
        ...
        try {
            boolean direExist = changeTargetDirectory(ftpClient, remoteDownLoadPath);
            if (!ftpClient.isConnected() || !ftpClient.isAvailable()) {
                System.out.println(">>>>>FTP服务器连接已经关闭或者连接无效*********");
                return false;
            }
            if (StringUtils.isBlank(localDires) || StringUtils.isBlank(remoteDownLoadPath)) {
     ...
     * @throws Exception
     */
    public static void uploadFiles(FTPClient ftpClient, File uploadFile, String remotePath) {
        /**如果 FTP 连接已经关闭，或者连接无效，则直接返回*/
        if (!ftpClient.isConnected() || !ftpClient.isAvailable()) {
            ...
            if (uploadFile.isDirectory()) {
                ...
                for (int i = 0; i < listFiles.length; i++) {
                    ...
                    if (loopFile.isDirectory()) {
                        /**如果有子目录，则迭代调用方法进行上传*/
                        uploadFiles(ftpClient, loopFile, remotePath);
    ...
    /**
     * @param client     ftp客户端
     * @param remotePath ftp目标路径
     * @return boolean   改变ftp路径是否成功
     */
    public static boolean changeTargetDirectory(FTPClient client, String remotePath) {
        String[] dires = remotePath.split("\\\\");
        boolean direExist = true;
        for (String dir : dires) {
            if (!"".equals(dir)) {
                boolean changeSuccessful = false;
                try {
                    changeSuccessful = client.changeWorkingDirectory(dir);
